"""
Parser para extractos PDF del Banco de Córdoba S.A. (BANCOR)
...
Notas:
  - BANCOR es el banco provincial de Córdoba. Muy usado por empresas cordobesas.
  - El formato es similar al BNA pero con algunos encabezados distintos.
  - BANCOR puede incluir retenciones IIBB específicas de Córdoba además de CABA:
    "PERC.IIBB CORDOBA" → se mapea también a PERC_IIBB_CABA (campo genérico).
    Si se necesita distinguir jurisdicción, extender los tipos de gasto.
  - Gastos: RET.SIRCREB, IMP.DB.CR.LEY 25413, COMISION, IVA, PERC.IIBB,
    IMP.SELLOS, INTERESES, PERC.IIBB CORDOBA.
"""
...
GASTOS_BANCARIOS = [
    ...
    # Córdoba y otras jurisdicciones → PERC_IIBB_CABA como campo genérico de IIBB
    (re.compile(r'PERC.*IIBB|IIBB.*CABA|IIBB.*CORD|PERCEPCION.*IIBB|IIBB', re.I), 'PERC_IIBB_CABA'),
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Uso: python parser_bancor.py <extracto.pdf>"); sys.exit(1)
