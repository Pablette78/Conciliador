"""
Parser para extractos PDF del Banco Supervielle Argentina
...
Formato típico:
  Cabecera:
    ...
    CUIT: 30-XXXXXXXX-X
    Cuenta Corriente N°: XXXXXXXXX   Sucursal: XXX
...
Gastos bancarios típicos Supervielle:
  - RET.SIRCREB                      → RET_SIRCREB
  - IMP.DB.CR.LEY 25413 DEB/CRED    → LEY25413_DEBITO / LEY25413_CREDITO
  - RETENCION IIBB TUCUMAN           → RET_IIBB_TUCUMAN
  - PERCEPCION IIBB CABA             → PERC_IIBB_CABA
  - COMISION MANTENIMIENTO           → COMISION
  - IVA S/COMISION                   → IVA
  - PERCEPCION IVA                   → PERC_IVA
  - IMP.SELLOS                       → IMP_SELLOS
  - INTERESES DESCUBIERTO            → INTERESES
"""
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Uso: python parser_supervielle.py <extracto.pdf>"); sys.exit(1)
