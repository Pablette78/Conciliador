"""
Parser para extractos PDF del Banco Patagonia S.A.
...
Notas:
  - Banco Patagonia es parcialmente propiedad del Banco do Brasil.
  - El formato del PDF es similar al de BNA pero con encabezados distintos.
  - Algunos extractos incluyen subtotales por tipo de operación al pie.
  - Gastos: RET.SIRCREB, IMP.DB.CR.LEY25413, COMISION, IVA, PERC.IIBB,
    IMP.SELLOS, INTERESES DESCTO.
"""
...
GASTOS_BANCARIOS = [
    ...
    (re.compile(r'INTER[EÉ]S(?:ES)?|INT\.?\s*DESC(?:TO)?', re.I),      'INTERESES'),
...
def parsear_pdf(ruta: str) -> dict:
    ...
    # Patagonia a veces incluye totales intermedios en líneas sin fecha
    ...
    for linea in lineas:
        ...
        if not ls:
            continue
        if RE_TOTAL.match(ls):
            continue  # saltar líneas de subtotales
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Uso: python parser_patagonia.py <extracto.pdf>"); sys.exit(1)
